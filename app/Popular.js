...
const Popular = async () => {
    ...
    return (
        <div className="trend w-full mt-4 max-h-fit ">
            <h1 className='block mb-4 text-2xl font-semibold'>Popular</h1>
            ...
            {trend.results.map(anime => (
                <Link href={`/info/${anime.id}`}><div className="card w-44 space-y-2" key={anime.id}>
                    ...
                    <h1 className='line-clamp-2'>{anime.title.userPreferred? anime.title.userPreferred:anime.title.romaji }</h1>
                </div></Link>
            ))}
            </div>
        </div>
