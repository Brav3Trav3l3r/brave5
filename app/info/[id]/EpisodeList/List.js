...
const List = async ({id}) => {
    ...
    return (
        <div  className="episodes p-4 mt-4 lg:mt-8 max-w-6xl mx-auto">
            <h1 className='mb-4 border-b-2 border-primary p-2'>Epsiodes</h1>
                ...
                {info.episodes.map(ep => (
                    <Link href={`/watch/${ep.id}`}><div className="card w-full p-1" key={ep.id}>
                        <div className="image w-full h-28 relative rounded-lg truncate">
                            <Image src={ep.image} fill={true} className='object-cover' alt={ep.id} />
                        </div>
                        <h1 className='title-third line-clamp-2' ><span className='text-primary'>{ep.number}.</span>{ep.title}</h1>
                    </div></Link>
                ))}
            </div>
        </div>
